...
The ESP32 Bluetooth RC Controller system is a modular embedded firmware platform designed to enable real-time remote control of an RC vehicle using bidirectional Bluetooth communication. The system is built around the ESP32 microcontroller and establishes a Serial Port Profile (SPP) Bluetooth link with an Android application, allowing both command transmission and telemetry feedback over a single wireless channel.

\subsubsection{What the System Does}

The system receives control commands from an Android application via Bluetooth SPP and translates them into hardware actions such as:
...
In addition to executing control commands, the firmware continuously gathers system and sensor data and transmits telemetry information back to the Android application. This telemetry may include panel values, indicators, plot data streams, input states, and sensor readings.

The communication is fully bidirectional, enabling real-time control and monitoring within the same wireless link.

\subsubsection{Who Uses the System}
...
\subsubsection{What Problem the System Solves}
...
\subsubsection{Bidirectional Bluetooth Communication}

The firmware establishes a Serial Port Profile (SPP) Bluetooth connection, enabling structured packet-based communication between the ESP32 and the Android application. This approach allows:
...
\subsubsection{Real-Time Control and Telemetry Feedback}
...
Simultaneously, telemetry modules collect data from sensors, input states, and internal system variables and transmit them back to the Android application. This creates a closed-loop control and monitoring environment suitable for experimentation, diagnostics, and performance tuning.

\subsubsection{Modular and Expandable Architecture}
...
\subsubsection{Deterministic Control Loop}
...
\subsubsection{Low-Latency Command Processing}

A primary objective of the system is to minimize the delay between user input on the Android application and hardware response on the vehicle.
...
\subsubsection{Robust Packet Synchronization}
...
\subsubsection{Modular Firmware Structure}
...
\subsubsection{Expandability and Experimentation}
...
The Android application acts as the human interface of the system. It generates control commands based on user interaction (such as joystick movement, button presses, or slider adjustments) and transmits them wirelessly to the ESP32.

In addition to sending commands, the application receives telemetry data from the vehicle, allowing real-time monitoring of system variables, indicators, and sensor readings.

\subsubsection{Bluetooth Communication Layer}
...
\subsubsection{Control Processing Layer}
...
\subsubsection{Telemetry Processing Layer}

The Telemetry Processing Layer gathers system information and prepares it for transmission back to the Android application.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		% Nodes
		\node[block] (android) {Android Application};
...
These two paths form a bidirectional communication loop between the Android application and the physical hardware connected to the ESP32. The downlink path is responsible for real-time control, while the uplink path provides monitoring and feedback.

\subsubsection{Downlink: Control Path}

The downlink path begins at the Android application, where user interactions such as joystick movements, button presses, and slider adjustments generate control data.

The functional flow can be summarized as:

\begin{center}
	Android Application $\rightarrow$ Bluetooth $\rightarrow$ Receiver $\rightarrow$ Control $\rightarrow$ Hardware
\end{center}

\textbf{1. Android Application} \\
The application encodes user input into structured control packets and transmits them over the Bluetooth Serial Port Profile (SPP).
...
\subsubsection{Uplink: Telemetry Path}
...
\begin{center}
	Hardware $\rightarrow$ Telemetry Source $\rightarrow$ Telemetry Module $\rightarrow$ Bluetooth $\rightarrow$ Android Application
\end{center}
...
\textbf{5. Android Application} \\
The Android application receives and decodes telemetry packets, updating graphical elements such as indicators, charts, and data panels in real time.

\subsubsection{Bidirectional Operation}
...
\subsection{Real-Time Characteristics}
...
\subsubsection{Polling-Based Architecture}
...
\subsubsection{Non-Blocking Loop Design}
...
\subsubsection{Avoidance of \texttt{delay()} Usage}
...
\subsubsection{Fixed-Size Packet Model for Control}
...
\subsubsection{Preparation for Timing Analysis}
...
\subsection{Modular Firmware Philosophy}
...
\begin{itemize}
	...
	\item \textbf{Packet Definitions} — Defines structured data formats for control and telemetry communication, ensuring consistent interpretation between the ESP32 and the Android application.
	...
	\item \textbf{Telemetry System} — Collects system data, formats telemetry packets, and transmits structured feedback to the Android application.
...
\subsubsection{Hardware Abstraction Separation}
...
\subsubsection{Scalability and Future Extension}
...
\subsubsection{Digital Output Control}
...
\subsubsection{Telemetry and Monitoring Features}

The system provides structured telemetry functionality to enable real-time monitoring from the Android application. Supported telemetry features include:
...
\begin{itemize}
	...
	\item \textbf{Input Telemetry Reporting} — Feedback of input states and system variables to the Android interface.
\end{itemize}
...
\subsubsection{Platform Flexibility}
...
\subsubsection{Bluetooth Bandwidth and Latency}
...
\subsubsection{Microcontroller Resource Limits}
...
\subsubsection{Fixed Buffer Sizes}
...
\subsubsection{Checksum Simplicity}
...
\subsubsection{Single-Loop Execution Model}
